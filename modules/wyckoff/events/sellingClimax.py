...
def detect_selling_climax(context: TimeframeContext) -> Dict[str, Any]:
    """
    Detect SELLING CLIMAX (SC) conditions:
    - Panic selloff / capitulation
    - Volume spike at lows
    - Sharp downside expansion
    - Immediate absorption / reversal attempt
    """
    ...
    for i in range(10, len(df) - 2):
        ...
        lower_wick = min(closes[i], opens[i]) - lows[i] if "Open" in df else closes[i] - lows[i]

        wick_expansion = lower_wick > (price_range[i] * 0.5)
        ...
        accumulation_signal = (
            is_climax_vol and
            is_expansion and
            next_move > 0  # reversal after panic selling
        )
    ...
    context.breakdowns.extend(events)

    return {
        "timeframe": context.timeframe,
        "selling_climax_count": len(events),
        "events": events
    }